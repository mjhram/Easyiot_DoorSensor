...
<tr>
	...
	<td><input type="text" id="devicename" name="devicename" value="" maxlength="16"></td>
</tr>
<tr>
...
void send_general_html(AsyncWebServerRequest *server)
{
	...
	if (server->params() > 0 )  // "Save" is clicked
	{
		...
		for ( uint8_t i = 0; i < server->params(); i++ ) {
			if (server->getParam(i)->name() == "devicename") urldecode(server->getParam(i)->value()).toCharArray(config.DeviceName,16); 
